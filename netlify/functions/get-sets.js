exports.handler = async (event) => {
  ...
  try {
    const response = await fetch(
      `https://rebrickable.com/api/v3/lego/parts/${encodeURIComponent(partNum)}/sets/?page_size=${pageSize}`,
      { headers: { 'Authorization': `key ${process.env.REBRICKABLE_API_KEY}` } }
    );

    if (!response.ok) {
      return { statusCode: response.status, body: JSON.stringify({ error: 'Sets not found' }) };
    }
